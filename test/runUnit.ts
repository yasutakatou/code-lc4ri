...
const Module = require('module') as { _resolveFilename: Function; _cache: any };
...
const ext = require('../src/extension') as typeof import('../src/extension');
...
function truthy(name: string, v: unknown): void {
    if (v) { passed++; console.log(`  ✓ ${name}`); }
    else   { failed++; console.log(`  ✗ ${name} (got ${String(v)})`); }
}
...
const cfg: import('../src/extension').LC4RIConfig = {
    timeout: 0, template: { linux: 'L({COMMAND})', win32: 'W({COMMAND})', darwin: 'D({COMMAND})' },
    profiles: { ssh: 'ssh host {COMMAND}' },
    changeWord: {}, toutf8: false, toterminal: false, outputFormat: 'codeblock' as const,
...
const osTemplate = cfg.template[process.platform as keyof typeof cfg.template];
if (osTemplate) {
...
eq('basic block',
    ext.collectFencedBlock(['```', 'line1', 'line2', '```'], 0),
    { content: 'line1\nline2', consumed: 4 }
);
eq('with lang tag',
    ext.collectFencedBlock(['```sh', 'echo hi', '```'], 0),
    { content: 'echo hi', consumed: 3 }
);
eq('indented fence strips indent',
    ext.collectFencedBlock(['  ```', '  hello', '  world', '  ```'], 0),
    { content: 'hello\nworld', consumed: 4 }
);
eq('skips leading blank lines',
    ext.collectFencedBlock(['', '  ', '```', 'content', '```'], 0),
    { content: 'content', consumed: 5 }
);
eq('no fence returns null',
    ext.collectFencedBlock(['just text', 'more text'], 0),
    { content: null, consumed: 0 }
);
eq('unclosed fence returns null',
    ext.collectFencedBlock(['```', 'line1'], 0),
    { content: null, consumed: 0 }
);
eq('empty block',
    ext.collectFencedBlock(['```', '```'], 0),
    { content: '', consumed: 2 }
);
eq('startIdx offset',
    ext.collectFencedBlock(['- write: foo', '```', 'body', '```'], 1),
...
console.log('buildStatusBarView');
{
    ...
    const states: import('../src/extension').Lc4riRunState[] = ['idle', 'running', 'ok', 'fail', 'noTarget', 'cancelled'];
    truthy('every state is labelled lc4ri',
        states.every(s => ext.buildStatusBarView(s).text.includes('lc4ri:')));
    truthy('every tooltip offers profile switch',
        states.every(s => ext.buildStatusBarView(s).tooltip.includes('Click to switch execution profile.')));
}
